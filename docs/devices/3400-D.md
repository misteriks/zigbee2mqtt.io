...
```js
{
    ...
    "mode": "arm_all_zones" // Mode "disarm", "arm_day_zones", "arm_night_zones", "arm_all_zones", "exit_delay", "entry_delay", "not_ready", "in_alarm", "arming_stay", "arming_night", "arming_away" or "invalid_code" (take this value from the (dis)arm attempt property `action`)
    }
}
```
